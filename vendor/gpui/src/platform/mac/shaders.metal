// Modified from upstream gpui 0.2.2 (Apache-2.0) — see MODIFICATIONS.md
// in this directory for what changed and why.
...
vertex LiquidGlassVertexOutput liquid_glass_vertex(
    ...
    uint material_id [[instance_id]],
    constant float2 *unit_vertices [[buffer(LiquidGlassInputIndex_Vertices)]],
    constant LiquidGlass *materials [[buffer(LiquidGlassInputIndex_Materials)]],
    constant Size_DevicePixels *viewport_size [[buffer(LiquidGlassInputIndex_ViewportSize)]]) {
  float2 unit_vertex = unit_vertices[unit_vertex_id];
  LiquidGlass material = materials[material_id];
  float4 clip_distance = distance_from_clip_rect(
    unit_vertex, material.bounds, material.content_mask.bounds);
  return LiquidGlassVertexOutput{
    material_id,
    to_device_position(unit_vertex, material.bounds, viewport_size),
    {clip_distance.x, clip_distance.y, clip_distance.z, clip_distance.w}
  };
}

float liquid_glass_sdf(float2 point, float2 half_size, float radius) {
  float2 q = abs(point) - (half_size - radius);
  return length(max(q, float2(0.0))) + min(max(q.x, q.y), 0.0) - radius;
}

float2 liquid_glass_normal(float2 point, float2 half_size, float radius) {
  const float epsilon = 0.5;
  float dx = liquid_glass_sdf(point + float2(epsilon, 0.0), half_size, radius)
           - liquid_glass_sdf(point - float2(epsilon, 0.0), half_size, radius);
  float dy = liquid_glass_sdf(point + float2(0.0, epsilon), half_size, radius)
           - liquid_glass_sdf(point - float2(0.0, epsilon), half_size, radius);
  return normalize(float2(dx, dy) + float2(1e-6));
}

float3 sample_liquid_gaussian(texture2d<float> source, sampler source_sampler,
                              float2 pixel, float2 viewport_size,
                              float2 axis, float scale) {
  float3 color = float3(0.0);
  float total = 0.0;
  float sigma = 8.0 * scale;
  float stride = 3.0 * scale;
  for (int tap = -8; tap <= 8; tap++) {
    float offset = float(tap) * stride;
    float weight = exp(-0.5 * (offset * offset) / (sigma * sigma));
    color += source.sample(source_sampler, (pixel + axis * offset) / viewport_size).rgb * weight;
    total += weight;
  }
  return color / total;
}

vertex LiquidGlassVertexOutput liquid_glass_blur_vertex(
    uint unit_vertex_id [[vertex_id]],
    uint material_id [[instance_id]],
    constant float2 *unit_vertices [[buffer(LiquidGlassInputIndex_Vertices)]],
    constant LiquidGlass *materials [[buffer(LiquidGlassInputIndex_Materials)]],
...
fragment float4 liquid_glass_blur_fragment(
    LiquidGlassFragmentInput input [[stage_in]],
    constant LiquidGlass *materials [[buffer(LiquidGlassInputIndex_Materials)]],
    constant Size_DevicePixels *viewport_size_raw [[buffer(LiquidGlassInputIndex_ViewportSize)]],
    texture2d<float> backdrop [[texture(LiquidGlassInputIndex_Backdrop)]]) {
  constexpr sampler backdrop_sampler(coord::normalized, address::clamp_to_edge, filter::linear);
  LiquidGlass material = materials[input.material_id];
  float2 viewport_size = float2(viewport_size_raw->width, viewport_size_raw->height);
  float3 color = sample_liquid_gaussian(
    backdrop, backdrop_sampler, input.position.xy, viewport_size,
    float2(1.0, 0.0), material.material_scale);
  return float4(color, 1.0);
}

float liquid_glass_shadow_alpha(LiquidGlass material, float2 position) {
  float2 origin = float2(material.shadow_bounds.origin.x, material.shadow_bounds.origin.y);
  float2 size = float2(material.shadow_bounds.size.width, material.shadow_bounds.size.height);
  float2 half_size = size * 0.5;
  float2 point = position - origin - half_size;
  float blur_radius = 24.0 * material.material_scale;
  float low = point.y - half_size.y;
  float high = point.y + half_size.y;
  float start = clamp(-3.0 * blur_radius, low, high);
  float end = clamp(3.0 * blur_radius, low, high);
  float step = (end - start) / 4.0;
  float y = start + step * 0.5;
  float alpha = 0.0;
  for (int i = 0; i < 4; i++) {
    alpha += blur_along_x(point.x, point.y - y, blur_radius,
                          material.corner_radius, half_size)
           * gaussian(y, blur_radius) * step;
    y += step;
  }
  return alpha * (56.0 / 255.0);
}

float3 shade_liquid_glass_sample(
    float3 color,
    bool clear,
    float edge,
    float2 normal,
  ...
  if (clear) {
    ...
    color = luma + (color - luma) * 1.05;
    color = color * 1.01 + 2.0 / 255.0;
  }

  float top_left_light = saturate((-normal.x - normal.y) * 0.5 + 0.5);
  float bright_rim = clear
    ? pow(edge, 6.0) * (0.12 + 0.58 * top_left_light)
    : pow(edge, 5.0) * (0.06 + 0.18 * top_left_light);
  float inner_glow = pow(edge, 1.7) * (clear ? 0.008 : 0.025);
  float dark_rim = pow(edge, 4.0) * (1.0 - top_left_light) * (clear ? 0.13 : 0.04);

  float3 body_tint = float3(1.0, 239.680115 / 255.0, 217.150872 / 255.0);
  float tint = (clear ? 0.0 : 0.56) + inner_glow + interaction * 0.025;
  color = mix(color, body_tint, tint);
  color = mix(color, float3(1.0), bright_rim) * (1.0 - dark_rim);

  if (!clear) {
    float3 fitted = float3(
...
fragment float4 liquid_glass_fragment(
    ...
    constant Size_DevicePixels *viewport_size_raw [[buffer(LiquidGlassInputIndex_ViewportSize)]],
    texture2d<float> backdrop [[texture(LiquidGlassInputIndex_Backdrop)]],
    texture2d<float> blurred [[texture(LiquidGlassInputIndex_Blur)]]) {
  constexpr sampler backdrop_sampler(coord::normalized, address::clamp_to_edge, filter::linear);
  LiquidGlass material = materials[input.material_id];
  float shadow_alpha = liquid_glass_shadow_alpha(material, input.position.xy);
  bool clear = material.variant == 1 || material.variant == 3;
  bool tinted = material.variant >= 2;
  float2 origin = float2(material.glass_bounds.origin.x, material.glass_bounds.origin.y);
  float2 size = float2(material.glass_bounds.size.width, material.glass_bounds.size.height);
  float2 half_size = size * 0.5;
  float2 point = input.position.xy - origin - half_size;
  float distance = -liquid_glass_sdf(point, half_size, material.corner_radius);
  if (distance < -0.5) {
    return float4(0.0, 0.0, 0.0, shadow_alpha * material.opacity);
  }

  float scale = material.material_scale;
  float bevel = (clear ? 16.0 : 15.0) * scale;
  float edge = saturate(1.0 - distance / bevel);
  float lens = smoothstep(0.0, 1.0, edge);
  float2 normal = liquid_glass_normal(point, half_size, material.corner_radius);
  float2 viewport_size = float2(viewport_size_raw->width, viewport_size_raw->height);
  float2 sample_pixel = input.position.xy - normal * (clear ? 26.0 : 3.2) * scale * lens;
  float3 sample = clear
    ? backdrop.sample(backdrop_sampler, sample_pixel / viewport_size).rgb
    : sample_liquid_gaussian(blurred, backdrop_sampler, sample_pixel, viewport_size,
                             float2(0.0, 1.0), scale);
  float3 color = shade_liquid_glass_sample(
    sample, clear, edge, normal, material.interaction);

  if (tinted) {
    float3 tinted_color = clear ? float3(
  ...
  // Cross-background fit to the active, undimmed Xcode-beta reference.
  if (material.variant == 1) {
    color = saturate(color);
    color = float3(
      1.334711 * color.r + 0.214589 * color.g - 0.281378 * color.b - 0.049184,
      -0.147284 * color.r + 1.629309 * color.g - 0.224290 * color.b - 0.039991,
      -0.196570 * color.r + 0.115755 * color.g + 1.324347 * color.b - 0.012006
    );
  } else if (material.variant == 3) {
    color = saturate(color);
    color = float3(
      1.472903 * color.r - 0.258041 * color.g - 0.097548 * color.b - 0.138594,
      1.510637 * color.r + 0.643022 * color.g - 0.306962 * color.b - 1.070967,
      1.930113 * color.r + 0.456934 * color.g - 0.273570 * color.b - 1.329566
    );
    color = saturate(color);
    color = float3(
      0.884876 * color.r - 0.007972 * color.g + 0.132201 * color.b + 0.042586,
      -0.222265 * color.r + 1.163902 * color.g + 0.052029 * color.b + 0.011485,
      -0.182171 * color.r + 0.156777 * color.g + 1.006060 * color.b + 0.013433
    );
  }

  float interaction_glow = material.interaction * (0.055 + pow(edge, 2.2) * 0.12);
  color = mix(color, float3(1.0), interaction_glow);
  float alpha = saturate(distance);
  float4 glass_color = float4(saturate(color) * alpha, alpha);
  float4 shadow_color = float4(0.0, 0.0, 0.0, shadow_alpha);
  float4 composite = glass_color + shadow_color * (1.0 - glass_color.a);
  // Keep the calibrated full-opacity edge response while fading both material
  // and shadow through GPUI's straight-alpha blend pipeline.
  composite.a *= material.opacity;
  return composite;
}
