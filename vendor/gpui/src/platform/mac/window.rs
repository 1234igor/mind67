...
use crate::{
    ...
    WindowBounds, WindowControlArea, WindowGlassAppearance, WindowGlassStyle, WindowKind,
...
impl Drop for MacWindowState {
    fn drop(&mut self) {
        if let Some(base_view) = self.glass_base_view.take() {
            unsafe {
                let _: () = msg_send![base_view, release];
            }
        }
    }
}
...
impl PlatformWindow for MacWindow {
    ...
    fn set_background_appearance(&self, background_appearance: WindowBackgroundAppearance) {
        ...
        let requested_glass = match background_appearance {
            WindowBackgroundAppearance::LiquidGlass(glass) => Some(glass),
            _ => None,
        };
        let native_glass_available = Class::get("NSGlassEffectView").is_some();
        let use_blur = background_appearance == WindowBackgroundAppearance::Blurred
            || (requested_glass.is_some() && !native_glass_available);
        ...
        unsafe {
            ...
            if NSAppKitVersionNumber < NSAppKitVersionNumber12_0 {
                ...
                let blur_radius = if use_blur { 80 } else { 0 };
                ...
                if let Some(glass) = requested_glass.filter(|_| native_glass_available) {
                    remove_blurred_view(&mut this);
                    let install_glass = this.glass_view.is_none();
                    drop(this);
                    if install_glass {
                        install_glass_view(&self.0);
                    }
                    let this = self.0.as_ref().lock();
                    configure_glass_view(this.glass_view.unwrap(), glass);
                    return;
                }

                let remove_glass = this.glass_view.is_some();
                drop(this);
                if remove_glass {
                    remove_glass_view(&self.0);
                }
                let mut this = self.0.as_ref().lock();

                // `NSVisualEffectView` is also the compatibility fallback when the app runs on a
                // pre-macOS 26 system where `NSGlassEffectView` is unavailable.
                if !use_blur {
                    remove_blurred_view(&mut this);
...
unsafe fn install_glass_view(state: &Arc<Mutex<MacWindowState>>) {
    unsafe {
        let glass_class = Class::get("NSGlassEffectView")
            .expect("NSGlassEffectView availability checked before installation");
        let (native_window, native_view) = {
            let this = state.lock();
            (this.native_window, this.native_view.as_ptr())
        };
        let base_view = native_window.contentView();
        let frame = NSView::frame(base_view);

        let mut glass_view: id = msg_send![glass_class, alloc];
        glass_view = NSView::initWithFrame_(glass_view, frame);
        glass_view.setAutoresizingMask_(NSViewWidthSizable | NSViewHeightSizable);

        // AppKit only guarantees the glass treatment for `contentView`. Reparent
        // the existing Metal-backed GPUI view without replacing its layer or
        // renderer, and make the glass the NSWindow root so AppKit owns its size.
        let _: id = msg_send![base_view, retain];
        let _: id = msg_send![native_view, retain];
        NSView::removeFromSuperview(native_view);
        let _: () = msg_send![glass_view, setContentView: native_view];
        let _: () = msg_send![glass_view, setFrame: frame];
        let _: () = msg_send![native_window, setContentView: glass_view];
        // Removing the GPUI view clears NSWindow.firstResponder. AppKit does not
        // restore it when the same view becomes NSGlassEffectView.contentView.
        let _: BOOL = native_window.makeFirstResponder_(native_view);
        debug_assert_eq!(
            native_window.firstResponder(),
            native_view,
            "GPUI view must remain first responder after installing window glass"
        );
        let _: () = msg_send![native_view, release];
        let mut this = state.lock();
        this.glass_view = Some(glass_view.autorelease());
        this.glass_base_view = Some(base_view);
    }
}

unsafe fn configure_glass_view(glass_view: id, glass: WindowGlassAppearance) {
    unsafe {
        let style: NSInteger = match glass.style {
            WindowGlassStyle::Regular => 0,
            WindowGlassStyle::Clear => 1,
        };
        let tint_color = glass.tint.map_or(nil, |tint| {
            NSColor::colorWithSRGBRed_green_blue_alpha_(
                nil,
                tint.r.clamp(0.0, 1.0) as f64,
                tint.g.clamp(0.0, 1.0) as f64,
                tint.b.clamp(0.0, 1.0) as f64,
                tint.a.clamp(0.0, 1.0) as f64,
            )
        });
        let _: () = msg_send![glass_view, setStyle: style];
        let _: () = msg_send![glass_view, setTintColor: tint_color];
        let _: () = msg_send![glass_view, setCornerRadius: glass.corner_radius.to_f64().max(0.0)];
    }
}

unsafe fn remove_glass_view(state: &Arc<Mutex<MacWindowState>>) {
    unsafe {
        let (native_window, native_view, glass_view, base_view) = {
            let mut this = state.lock();
            let Some(glass_view) = this.glass_view.take() else {
                return;
            };
            let base_view = this
                .glass_base_view
                .take()
                .expect("glass base view is retained while glass is installed");
            (
                this.native_window,
                this.native_view.as_ptr(),
                glass_view,
                base_view,
            )
        };
        let _: id = msg_send![native_view, retain];
        let _: () = msg_send![glass_view, setContentView: nil];
        let _: () = msg_send![native_window, setContentView: base_view];
        let _: () = msg_send![native_view, setTranslatesAutoresizingMaskIntoConstraints: YES];
        native_view.setAutoresizingMask_(NSViewWidthSizable | NSViewHeightSizable);
        let _: () = msg_send![native_view, setFrame: NSView::bounds(base_view)];
        base_view.addSubview_(native_view);
        let _: () = msg_send![native_view, release];
        let _: () = msg_send![base_view, release];
        native_window.makeFirstResponder_(native_view);
    }
...
unsafe fn remove_blurred_view(this: &mut MacWindowState) {
    unsafe {
