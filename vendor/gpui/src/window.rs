...
use crate::{
    ...
    LineLayoutIndex, LiquidGlass, Modifiers, ModifiersChangedEvent, MonochromeSprite, MouseButton,
...
impl Window {
    ...
    pub fn paint_shadows(
        ...
        let scale_factor = self.scale_factor();
        let content_mask = self.content_mask();
        let opacity = self.element_opacity().clamp(0.0, 1.0);
    ...
    /// Paints a live liquid-glass backdrop material over everything already drawn beneath it.
    ///
    /// The macOS renderer performs the refraction and shading on the GPU without reading the
    /// window contents back to the CPU. [`LiquidGlassVariant::Identity`] is a no-op.
    pub fn paint_liquid_glass(&mut self, glass: PaintLiquidGlass) {
        self.invalidator.debug_assert_paint();
        if glass.variant == LiquidGlassVariant::Identity {
            return;
        }
        ...
        let content_mask = self.content_mask();
        let opacity = self.element_opacity().clamp(0.0, 1.0);
        let max_corner_radius = (glass.bounds.size.width * 0.5)
            .min(glass.bounds.size.height * 0.5)
            .max(px(0.0));
        let corner_radius = glass.corner_radius.clamp(px(0.0), max_corner_radius);
        let shadow_bounds = (glass.bounds + point(px(0.0), px(8.0))).dilate(px(-6.0));
        let paint_bounds = shadow_bounds.dilate(px(72.0)).union(&glass.bounds);
        self.next_frame.scene.insert_primitive(LiquidGlass {
            order: 0,
            variant: glass.variant as u32,
            bounds: paint_bounds.scale(scale_factor),
            glass_bounds: glass.bounds.scale(scale_factor),
            shadow_bounds: shadow_bounds.scale(scale_factor),
            content_mask: content_mask.scale(scale_factor),
            corner_radius: corner_radius.scale(scale_factor),
            interaction: glass.interaction.clamp(0.0, 1.0),
            opacity,
            material_scale: px(1.0).scale(scale_factor),
        });
    }
...
/// A live GPU-backed liquid-glass backdrop material.
#[derive(Clone, Copy, Debug, Default, Eq, PartialEq)]
#[repr(u32)]
pub enum LiquidGlassVariant {
    /// The standard frosted, refractive material.
    #[default]
    Regular = 0,
    /// A more transparent and strongly refractive material.
    Clear = 1,
    /// The standard material with an application tint.
    RegularTinted = 2,
    /// The clear material with an application tint.
    ClearTinted = 3,
...
/// A live GPU-backed liquid-glass backdrop material.
#[derive(Clone)]
pub struct PaintLiquidGlass {
    /// The bounds of the material within the window.
    pub bounds: Bounds<Pixels>,
    /// The shared radius of all four corners.
    pub corner_radius: Pixels,
    /// The optical material to paint.
    pub variant: LiquidGlassVariant,
    /// Interaction intensity from rest (0) to pressed (1).
    pub interaction: f32,
}
