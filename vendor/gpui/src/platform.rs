...
use crate::{
    ...
    Point, RenderGlyphParams, RenderImage, RenderImageParams, RenderSvgParams, Rgba, Scene,
...
static LIQUID_GLASS_ENABLED: AtomicBool = AtomicBool::new(false);

/// Configures subsequently created windows for live Liquid Glass rendering.
///
/// Call this once before creating the application or opening any windows that
/// will paint [`crate::PaintLiquidGlass`] primitives.
pub fn enable_liquid_glass() {
    LIQUID_GLASS_ENABLED.store(true, Ordering::Relaxed);
}

#[allow(dead_code)]
pub(crate) fn liquid_glass_enabled() -> bool {
    LIQUID_GLASS_ENABLED.load(Ordering::Relaxed)
}
use std::{
    fmt::{self, Debug},
    ops::Range,
...
pub enum WindowBackgroundAppearance {
    ...
    Blurred,
    /// Native Liquid Glass that samples the compositor contents behind the window.
    ///
    /// macOS 26 and later use [`NSGlassEffectView`](https://developer.apple.com/documentation/appkit/nsglasseffectview).
    /// Earlier macOS releases fall back to the system blur; other platforms
    /// fall back to a transparent window background.
    LiquidGlass(WindowGlassAppearance),
}

/// The optical style used by a full-window Liquid Glass background.
#[derive(Copy, Clone, Debug, Default, PartialEq, Eq)]
pub enum WindowGlassStyle {
    /// Standard adaptive glass with a softer, more diffused appearance.
    #[default]
    Regular,
    /// Clear adaptive glass with stronger lensing and less diffusion.
    Clear,
}

/// Native full-window Liquid Glass configuration.
#[derive(Copy, Clone, Debug, PartialEq)]
pub struct WindowGlassAppearance {
    /// The native material style.
    pub style: WindowGlassStyle,
    /// Optional adaptive tint applied by the native material.
    pub tint: Option<Rgba>,
    /// Curvature of the glass view inside the native window.
    pub corner_radius: Pixels,
}

impl WindowGlassAppearance {
    /// Creates untinted Regular glass with no additional corner curvature.
    pub const fn regular() -> Self {
        Self {
            style: WindowGlassStyle::Regular,
            tint: None,
            corner_radius: Pixels::ZERO,
        }
    }

    /// Creates untinted Clear glass with no additional corner curvature.
    pub const fn clear() -> Self {
        Self {
            style: WindowGlassStyle::Clear,
            tint: None,
            corner_radius: Pixels::ZERO,
        }
    }

    /// Applies a native adaptive tint.
    pub const fn tint(mut self, tint: Rgba) -> Self {
        self.tint = Some(tint);
        self
    }

    /// Sets the glass corner radius in logical pixels.
    pub const fn corner_radius(mut self, corner_radius: Pixels) -> Self {
